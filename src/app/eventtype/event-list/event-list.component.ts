...
`;
  ...
  isMarketLive(marketId) {
    var liveMarket = this.liveMarkets
      .map(function (x) {
        return x.marketId;
      })
      .indexOf(marketId);
  ...
  getFencyBet(eventType, eventId, hasInPlayFancyBetMarkets) {
    this.bookMarkets=[];
      this.fencyMarkets=[];
    if (hasInPlayFancyBetMarkets) {
      
      this.skyService.getFencyBet(eventType, eventId).subscribe((x: any) => {
        this.fencyMarkets=x.fancyBetMarkets;
        console.log(x);
      });
    }
    
    if (hasInPlayFancyBetMarkets) {
      this.skyService.getBookmakerMarket(eventType, eventId).subscribe((x: any) => {
        this.bookMarkets=x.bookMakerMarket.markets;
      });
    }
  ...
  getevent(cid, i) {
    if (this.data[i].event == undefined) {
      ...
      this.skyService.listEventsByCompetitionId(this.id, cid).subscribe((y) => {
        y.sort((a, b) => {
          return (
            <any>new Date(b.event.openDate) - <any>new Date(a.event.openDate)
          );
        });
        this.data[i]["event"] = y;
        this.showLoad = false;
  ...
  getmarket(eid, i, j) {
    if (this.data[i].event[j].market == undefined) {
      ...
      this.skyService.listMarketsForEvent(eid).subscribe((z) => {
        ...
        z.forEach((element) => {
          var index = this.liveMarkets.findIndex(
          ...
          if (index < 0) {
            ...
            element["active"] = element["active"] =
              this.liveMarkets[index].enabled;
          }
          this.showLoad = false;
        });
  ...
  enableMarket(): any {
    ...
    this.anEventData["mtype"] = this.mtype;
    //console.log(this.anEventData);
    this.closebutton.nativeElement.click();
    return this.skyService.enableMarket(
      this.eventTypeData,
      this.aCompetitionData,
      this.anEventData,
      this.aMarketData,
      this.selected
    );
  ...
  saveMarkets(): any {
    ...
    return this.skyService.saveMarkets().then((resp) => {
      ...
      if (resp.add_result.marketsToAdd != "nothing to add") {
        ...
        alert(alertmsg);
        setTimeout(function () {
          window.location.href = "eventtype/eventlist/" + eid;
        }, 3000);
      } else {
